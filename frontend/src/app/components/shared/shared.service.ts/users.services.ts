...
export class UsersServices {
  ...
  filterUsers(isApproved:number | null, allUsers:IUser[], maxId:number):IUser[]{
    const filteredUsers = allUsers.filter(user=>{
      return ((user.user_approved === isApproved) && (user.roles.some(role=>{return role.role_id === maxId })) && (!user.roles.some(role=>{return role.role_id > maxId })))
    })
