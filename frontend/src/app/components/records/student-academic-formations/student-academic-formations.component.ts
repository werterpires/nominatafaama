...
export class StudentAcademicFormationsComponent {
  ...
  ngOnInit() {
    this.isLoading = true
    this.stAcademicFormationService
      .findAllStudentAcademicFormations()
      .subscribe({
        next: (res) => {
  ...
  createStAcademicFormation() {
    this.isLoading = true
    this.createStAcademicFormationData.degree_id = Number(
      this.createStAcademicFormationData.degree_id,
    )
    console.log(this.createStAcademicFormationData)
    this.stAcademicFormationService
      .createStudentAcademicFormation(this.createStAcademicFormationData)
      .subscribe({
        next: (res) => {
  ...
  editStAcademicFormation(i: number, buttonId: string) {
    ...
    const editFormationData: IStUpdateAcademicFormation = {
      ...
      conclusion_date: this.allFormations[i].conclusion_date
        ? this.allFormations[i].conclusion_date
        : null,
      formation_id: this.allFormations[i].formation_id,
    }

    this.stAcademicFormationService
      .editStudentAcademicFormation(editFormationData)
      .subscribe({
        next: (res) => {
  ...
  deleteRegistry(i: number) {
    this.isLoading = true
    const associationId = this.allFormations[i].formation_id

    this.stAcademicFormationService.deleteRegistry(associationId).subscribe({
      next: (res) => {
