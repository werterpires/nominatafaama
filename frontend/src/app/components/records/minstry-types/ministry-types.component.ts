...
import { IMinistryType, ICreateMinistryTypeDto, IUpdateMinistryType } from './types';
...
export class MinistryTypesComponent {
  ...
  createMinistryType() {
    this.isLoading = true;
    this.ministryTypesService.createMinistryType(this.createMinistryTypeData).subscribe({
      next: res => {
