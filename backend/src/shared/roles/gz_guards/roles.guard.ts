...
export class RolesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    const restrictRoles = this.reflector.getAllAndOverride<ERoles[]>('restrictRoles', [
      context.getHandler(),
      context.getClass(),
    ]);
    ...
    if (userId !== user.user_id){
      throw new ForbiddenException('Esse recurso só pode ser acessado pelo próprio usuário')
    }
