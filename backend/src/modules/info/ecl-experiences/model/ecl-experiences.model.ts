...
export class EclExperiencesModel {
  ...
  async createEclExperience(createEclExperienceData: ICreateEclExperience): Promise<IEclExperience> {
    let eclExperience: IEclExperience | null = null;
    let sentError: Error | null = null;

    await this.knex.transaction(async (trx) => {
      try {
        const { person_id, ecl_exp_type_id, ecl_exp_approved } = createEclExperienceData;
  ...
  async findEclExperienceById(id: number): Promise<IEclExperience | null> {
    ...
    await this.knex.transaction(async (trx) => {
      try {
        ...
        .leftJoin('ecl_exp_types', 'ecl_experiences.ecl_exp_type_id', 'ecl_exp_types.ecl_exp_type_id')
  ...
  async findEclExperiencesByPersonId(personId: number): Promise<IEclExperience[]> {
    let eclExperiencesList: IEclExperience[] = [];
    let sentError: Error | null = null;
  ...
  async updateEclExperienceById(updateEclExperience: IUpdateEclExperience): Promise<IEclExperience> {
  ...
  async updateEclExperiences(input:IUpdateEclExperiences): Promise<void> {
    ...
    await this.knex.transaction(async (trx) => {
      try {
        ...
        const newEclExpTypeIds = ecl_exp_type_ids.filter((id) => !existingEclExpTypeIds.includes(id));
