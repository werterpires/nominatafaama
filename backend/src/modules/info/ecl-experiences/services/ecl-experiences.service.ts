...
import { ICreateEclExperience, IEclExperience, IUpdateEclExperience, IUpdateEclExperiences } from '../types/types';
...
export class EclExperiencesService {
  ...
  async createEclExperience(dto: CreateEclExperienceDto, id:number): Promise<IEclExperience> {
    try {
      ...
      const createEclExperience: ICreateEclExperience = {
        person_id: person_id,
        ecl_exp_type_id: dto.ecl_exp_type_id,
        ecl_exp_approved: null
    
      };

      const newEclExperience = await this.eclExperiencesModel.createEclExperience(createEclExperience);
  ...
  async findEclExperienceById(id: number): Promise<IEclExperience | null> {
    try {
      const eclExperience = await this.eclExperiencesModel.findEclExperienceById(id);
  ...
  async findEclesiasticExperienceByPersonId(id: number ): Promise<IEclExperience[] | null> {
    try {

      const {person_id} = await this.usersService.findUserById(id)
      console.log(person_id)

      const eclExperience = await this.eclExperiencesModel.findEclExperiencesByPersonId(person_id);
      return eclExperience;
    } catch (error) {
  ...
  async findAllEclExperiences(): Promise<IEclExperience[]> {
    try {
      const allEclExperiences = await this.eclExperiencesModel.findAllEclExperiences();
  ...
  async updateEclExperienceById(dto: UpdateEclExperienceDto): Promise<IEclExperience> {
    try {
  
      const updatedEclExperience: IUpdateEclExperience = {
        ecl_exp_id: dto.ecl_exp_id,
        ecl_exp_type_id: dto.ecl_exp_type_id
      };

      const eclesiasticExperiences = await this.eclExperiencesModel.updateEclExperienceById(updatedEclExperience);
  ...
  async updateEclExperiences(dto: UpdateExperiencesDto, id:number): Promise<void> {
    try {
      const {person_id} = await this.usersService.findUserById(id)
      ...
      await this.eclExperiencesModel.updateEclExperiences(updatedEclExperience);
