...
export class EndowmentTypesService {
  ...
  async createEndowmentType(
    ...
    try {
      ...
      const newEndowmentType =
        await this.endowmentTypesModel.createEndowmentType(
          endowmentType,
          currentUser
        )
  ...
  async findEndowmentTypeById(id: number): Promise<IEndowmentType> {
    try {
      const endowmentType =
        await this.endowmentTypesModel.findEndowmentTypeById(id)
      return endowmentType as IEndowmentType
    } catch (error) {
      throw new Error(
        `Failed to find endowment type with id ${id}: ${error.message}`
      )
    }
  }
  ...
  async findAllEndowmentTypes(): Promise<IEndowmentType[]> {
    try {
      const endowmentTypes =
        await this.endowmentTypesModel.findAllEndowmentTypes()
  ...
  async updateEndowmentTypeById(
    ...
    try {
      updatedEndowmentType =
        await this.endowmentTypesModel.updateEndowmentTypeById(
          input,
          currentUser
        )
    } catch (error) {
  ...
  async deleteEndowmentTypeById(
    ...
    try {
      const message = await this.endowmentTypesModel.deleteEndowmentTypeById(
        id,
        currentUser
      )
      return message
  ...
  async findEndowmentTypesByApplicationType(
    ...
    try {
      if (applicationType === 'student') {
        endowmentTypes =
          await this.endowmentTypesModel.findEndowmentTypesByCategory(
            applicationType
          )
