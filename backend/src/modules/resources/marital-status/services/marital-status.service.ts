...
export class MaritalStatusService {
  ...
  async createMaritalStatus(dto: CreateMaritalStatusDto): Promise<IMaritalStatus> {
    try {
      const maritalStatus: ICreateMaritalStatus = {
        marital_status_type_name: dto.marital_status_type_name,
      };
  
      const newMaritalStatus = await this.maritalStatusModel.createMaritalStatus(maritalStatus);
  ...
  async findMaritalStatusById(id: number): Promise<IMaritalStatus> {
    try {
      const maritalStatus = await this.maritalStatusModel.findMaritalStatusById(id);
      return maritalStatus as IMaritalStatus;
    } catch (error) {
      throw new Error(`Não foi possível encontrar o marital status com id ${id}: ${error.message}`);
    }
  }
  
  async findAllMaritalStatuses(): Promise<IMaritalStatus[]> {
    try {
      const maritalStatuses = await this.maritalStatusModel.findAllMaritalStatus();
  ...
  async deleteMaritalStatusById(id: number): Promise<string> {
    try {
      const message = await this.maritalStatusModel.deleteMaritalStatusById(id);
      return message;
    } catch (error) {
      throw error;
    }
