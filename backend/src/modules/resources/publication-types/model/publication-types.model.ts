...
import { IUpdateUnion } from '../../unions/types/types';

@Injectable()
export class PublicationTypeModel {
  constructor(@InjectModel() private readonly knex: Knex) {}

  async createPublicationType({ publication_type, instructions }: ICreatePublicationType): Promise<IPublicationType> {
  ...
  async findPublicationTypeById(id: number): Promise<IPublicationType | null> {
    ...
    await this.knex.transaction(async (trx) => {
      try {
        ...
        publicationType = {
          publication_type_id: result.union_id,
    ...
    await this.knex.transaction(async (trx) => {
      try {
        ...
        publicationTypeList = results.map((row: any) => ({
          publication_type_id: row.union_id,
  ...
  async updatePublicationTypeById(updatePublicationType: IUpdatePublicationType): Promise<IPublicationType> {
    let updatedPublicationType: IPublicationType | null = null;
    let sentError: Error | null = null;
  
    await this.knex.transaction(async (trx) => {
      try {
        const { publication_type, instructions, publication_type_id } = updatePublicationType;
  
        await trx('publication_types')
          .where('publication_type_id', publication_type_id)
          .update({ publication_type, instructions });
  
        updatedPublicationType = await this.findPublicationTypeById(publication_type_id);
  ...
  async deletePublicationTypeById(id: number): Promise<string> {
    ...
    await this.knex.transaction(async (trx) => {
      try {
        await trx('publication_types').where('publication_types_id', id).del();
