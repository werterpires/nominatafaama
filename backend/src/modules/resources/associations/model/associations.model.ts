...
export class AssociationsModel {
  ...
  async findAllAssociations(): Promise<IAssociation[]> {
    ...
    async updateAssociationById(updateAssociation: IUpdateAssociation): Promise<IAssociation> {
        let updatedAssociation: IAssociation | null = null;
        let sentError: Error | null = null;
      
        await this.knex.transaction(async (trx) => {
            try {
                const { association_name, association_acronym, association_id, union_id } = updateAssociation;
