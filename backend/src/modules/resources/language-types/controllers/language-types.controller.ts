...
export class LanguageTypesController {
  ...
  async createLanguageType(@Body() input: CreateLanguageTypeDto) {
    try {
      const languageType = await this.languageTypesService.createLanguageType(input);
      return languageType;
    } catch (error) {
  ...
  async getLanguageTypeById(@Param('id') id: number) {
    try {
      const languageType = await this.languageTypesService.findLanguageTypeById(id);
      if (!languageType) {
  ...
  async updateLanguageType(@Body() input: UpdateLanguageTypeDto) {
    try {
      const updatedLanguageType = await this.languageTypesService.updateLanguageTypeById(input);
