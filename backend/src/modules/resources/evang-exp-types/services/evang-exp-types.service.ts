...
import { IEvangExpType, ICreateEvangExpType, IUpdateEvangExpType } from '../types/types';

@Injectable()
export class EvangExpTypesService {
  constructor(private evangExpTypesModel: EvangExpTypesModel) {}

  async createEvangExpType(dto: CreateEvangExpTypeDto): Promise<IEvangExpType> {
    try {
      const evangExpType: ICreateEvangExpType = {
        evang_exp_type_name: dto.evang_exp_type_name,
      };

      const newEvangExpType = await this.evangExpTypesModel.createEvangExpType(evangExpType);
  ...
  async findEvangExpTypeById(id: number): Promise<IEvangExpType> {
    try {
      const evangExpType = await this.evangExpTypesModel.findEvangExpTypeById(id);
      return evangExpType as IEvangExpType;
    } catch (error) {
      throw new Error(`Failed to find evangelical experience type with id ${id}: ${error.message}`);
    }
  }
  ...
  async updateEvangExpTypeById(input: IUpdateEvangExpType): Promise<IEvangExpType> {
    let updatedEvangExpType: IEvangExpType | null = null;
    let sentError: Error | null = null;

    try {
      updatedEvangExpType = await this.evangExpTypesModel.updateEvangExpTypeById(input);
    } catch (error) {
  ...
  async deleteEvangExpTypeById(id: number): Promise<string> {
    try {
      const message = await this.evangExpTypesModel.deleteEvangExpTypeById(id);
      return message;
    } catch (error) {
      throw error;
    }
