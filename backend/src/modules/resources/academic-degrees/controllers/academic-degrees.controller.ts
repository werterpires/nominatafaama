...
export class AcademicDegreesController {
  ...
  async createAcademicDegree(@Body() input: CreateAcademicDegreeDto) {
    try {
      const academicDegree =
        await this.academicDegreesService.createAcademicDegree(input)
  ...
  async updateAcademicDegreeById(
    ...
    try {
      const updatedAcademicDegree =
        await this.academicDegreesService.updateAcademicDegreeById(input)
  ...
  async deleteAcademicDegreeById(@Param('id') id: number) {
    try {
      const message =
        await this.academicDegreesService.deleteAcademicDegreeById(id)
      return { message }
    } catch (error) {
      throw new InternalServerErrorException(error.message)
    }
