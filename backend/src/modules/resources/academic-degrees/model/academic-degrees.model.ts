...
export class AcademicDegreesModel {
  ...
  async createAcademicDegree({ degree_name }: ICreateAcademicDegree): Promise<IAcademicDegree > {
    let academicDegree: IAcademicDegree | null = null;
    let sentError: Error | null = null;
  
    await this.knex.transaction(async (trx) => {
      try {
        const [result] = await trx('academic_degrees').insert({
          degree_name,
        });
  ...
  async updateAcademicDegreeById(updateAcademicDegree: IUpdateAcademicDegree): Promise<IAcademicDegree> {
